...
public class CustomWebMvcRegistration implements WebMvcRegistrations {
	...
	private String multipartFileToString(MultipartFile file) {
		return FILE_START + NAME + EQUAL + file.getName() + COMMA + ORIGINAL_FILE_NAME + EQUAL
				+ file.getOriginalFilename() + COMMA + CONTENT_TYPE + EQUAL + file.getContentType() + COMMA + SIZE
				+ EQUAL + file.getSize() + RIGHT_BRACKET;
	}
